...
The original CAINE component: a small, honestly-untrained neural network.
There is no pretrained checkpoint anywhere in this directory -- a fresh
instance starts as random Gaussian noise and produces gibberish. The only
way it gets better at anything is by talking to it: every message sent is
used as an online training example (truncated backprop through time) the
moment it replies.

This is a pure text chatbot -- it has no connection to the
[Minecraft agent](../minecraft-agent) living alongside it in this repo. See
the [repo root README](../README.md) for how the two pieces relate.
...
- `caine/server.py` -- a small asyncio TCP server that speaks a line-based
  protocol. Each line in, CAINE replies, then learns from the exchange
